# pypong
>### I built the pong game with python <br><br>
>  I though it could be a fun way of refreshing my python skills.  <br><br>
...
```sh
git clone git@github.com:t-starkw/pypong.git
```

Ensure that you have the latest Python version and an interpreter installed on your local device.


## How To Play

- Right click on the  pong.py file and select `Run Python File in Terminal`
- Have one player control the left paddle by shifting it up or down with the `W` and `S` keys
- Have another player control the right paddle with the `Up` and `Down` keys
- Keep the pong ball in volley for as long as possible
...
1. Fork it (<https://github.com/t-starkw/pypong/fork>)
